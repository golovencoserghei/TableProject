...
if($a==1)
{
 ...
 if (count($post_array) != 0){
          ...
          $tbid=$_COOKIE['tbl'];
            ...
            $addlog=mysql_query("INSERT INTO `add_logo` VALUES (NULL,'".$user_arr[0]."','".$post_userid."','".$tbid."','".$post_dateid."','".$post_time."','".date("d.m.Y H:i")."','0')");
            ...
            $addlog=mysql_query("INSERT INTO `add_logo` VALUES (NULL,'".$user_arr[0]."','".$post_userid."','".$tbid."','".$post_dateid."','".$post_time."','".date("d.m.Y H:i")."','0')");
						...
						if($query) {
							...
							if ($get_array['pag']=='nw'){
								$fixdid = dateprotect($get_array['fid']);
								$ind='indexwn.html#'.$fixdid;
									header("location: $ind");
								}
								else{
									$fixdid = dateprotect($get_array['fid']);
									$ind='indexn.html#'.$fixdid;
									header("location: $ind");
								}

							}
							else msg("Ошибка при удалении записи!","wr");
											...
											<div class='form-group'>
												...
												<div class='container'>
													<center>Удалить запись
															<?
															$date_dl=mysql_fetch_array(mysql_query("SELECT `date` FROM `tb_date` WHERE `id`='".$dateid."'"));
