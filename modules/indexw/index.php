...
?>
					...
					<ul class="nav nav-tabs  nav-tabs-line">
                                    <li class="nav-item">
                                        <a href="index.html" class="nav-link"><? echo $st_thw;?></a>
                                    </li>
                                    <li class="nav-item">
	...
	while($row=mysql_fetch_array($site_arr))
	{
		...
		if($nsttyme==0){
			$one=mysql_fetch_array(mysql_query("SELECT `su_time`,`mo_time`,`tu_time`,`we_time`,`th_time`,`fr_time`,`sa_time`,`su_time` FROM `tb_congr` WHERE `id`='$user_arr[6]'"));
			 ...
			 $one=mysql_fetch_array(mysql_query("SELECT `su_time`,`mo_time`,`tu_time`,`we_time`,`th_time`,`fr_time`,`sa_time`,`su_time` FROM `tb_congr` WHERE `id`='62'"));
   ...
   ");
    ...
    foreach($one as $key => $value)
	{
		   ...
		   echo("
				...
				if($f_u[1]!=0){
					 if($cfg[0]==1 or $user_arr[4]!=1){
						echo("<a href='index.html&sub=delete&use=".$f_u[1]."&did=".$row[0]."&tid=".$key."&fid=".$row[2]."&pag=nw&ti=".str_replace([':'], ['f'], $value)."' alt='Выписать' title='Выписать' border='0'><img src='template/apimages/delete.png' width='20' height='20'></a>");
					 }}
		...
		echo("
		  ...
		  if($f_u[0] == 0) {
			 echo("
			        <td><a  href='index.html&sub=partner&did=".$row[0]."&tid=".$key."&fid=".$row[2]."&pag=nw&ti=".str_replace([':'], ['f'], $value)."'><center><span style='color: green;'><i class='fa fa-plus' title='Записать напарника'></i></span></center></a>
			     </td>
			     ");
		  }
...
if($cfg[0]==1 or $user_arr[4]!=1){
						...
						echo("<a href='index.html&sub=delete&use=".$f_u[0]."&did=".$row[0]."&tid=".$key."&fid=".$row[2]."&pag=nw&ti=".str_replace([':'], ['f'], $value)."	' alt='Выписать' title='Выписать' border='0'><img src='template/apimages/delete.png' width='20' height='20'></a>");
			...
			if($f_u[1] == 0 ) {
			 echo("
			        <td><a  href='index.html&sub=partner&did=".$row[0]."&tid=".$key."&fid=".$row[2]."&pag=nw&ti=".str_replace([':'], ['f'], $value)."'><center><span style='color: green;'><i class='fa fa-plus' title='Записать напарника'></i></span></center></a>  </td>
			     ");
			}
			else {
			  echo("
  ...
  echo("</tr>");
	...
	echo("<a  href='index.html&sub=raport&did=".$row[0]."&tid=".$key."'><i class='icon icon-notebook2 green-text s-18'></i></a>");
	 ...
	 echo("
      ...
      <td bgcolor='#90EE90'> <a href='tel:".$user0[2]."' alt='Позвонить' title='Позвонить' border='0'><span style='color: green;'><i class='fa fa-phone-square'></i></span></a> &nbsp; ".$user0[0]."");
	  ...
	  echo("</td>
	  <td bgcolor='#90EE90'> <a href='tel:".$user1[2]."' alt='Позвонить' title='Позвонить' border='0'><span style='color: green;'><i class='fa fa-phone-square'></i></span></a> &nbsp;".$user1[0]."");
