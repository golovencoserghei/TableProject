...
if($user_arr['stand']==0)
{
	...
	<ul class="nav nav-tabs  nav-tabs-line">
        <li class="nav-item">
			        <a href="#thisw" data-toggle="tab" class="nav-link active"><? echo $st_thw;?></a>
        </li>
        <li class="nav-item">
            <a href="indexw.html" class="nav-link"><? echo $st_nxw;?></a>
        </li>
        <li class="nav-item">
     ...
     if($f_u[0] == 0 or $f_u[1] == 0 ) {
              ...
              if($f_u[0] == 0) {
                  ...
                  if($f_u[1]!=0){
                    ...
                    echo("<a href='stand.html&sub=mdl&use=".$f_u[1]."&did=".$row[0]."&tid=".$key."&fid=".$row[2]."&ti=".str_replace([':'], ['f'], $value)."&token=".substr(md5($f_u[0].$row[0].$key.$row[2].str_replace([':'], ['f'], $value)),1,10)."' alt='Выписать' title='Выписать' border='0'><img src='template/apimages/delete.png' width='20' height='20'></a>");
					  }
                  }
                  echo("</td>");
                  ...
                  if($f_u[0] == 0) {
                    echo("<td><a  href='stand.html&sub=partner&did=".$row[0]."&tid=".$key."&fid=".$row[2]."&ti=".str_replace([':'], ['f'], $value)."&token=".substr(md5($row[0].$key.$row[2].str_replace([':'], ['f'], $value)),1,10)."'><center><span style='color: green;'><i class='fa fa-plus' title='Записать напарника'></i></span></center></a></td>");
                  } 
				  
				  
				  if($cfg[1] == 1) {
					  if($user_arr[4]>1 ){
						  echo("<td><a  href='stand.html&sub=raport&did=".$row[0]."&tid=".$key."&ti=".str_replace([':'], ['f'], $value)."'><center><span style='color: green;'><i class='fa fa-pencil'></i></span></center></a></td>");
                     
					  }
					  else{
					  if($f_u[0] == $user_arr[0] or $f_u[1] == $user_arr[0]){
                       echo("<td><a  href='index.html&sub=raport&did=".$row[0]."&tid=".$key."&ti=".str_replace([':'], ['f'], $value)."'><center><span style='color: green;'><i class='fa fa-pencil'></i></span></center></a></td>");
                     }
					 else{
						 echo"<td></td>";
              ...
              elseif($f_u[1] == 0) {
                  ...
                  echo("<tr><th>".$value." </th><td"); if ($f_u[0]>0){ echo(" bgcolor='#FFA07A'"); } echo(">".$us_name[0]." ");

				              if($f_u[0]!=0){
								 if($cfg[0]==1 or $user_arr[4]!=1){
								  
                        echo("<a href='stand.html&sub=mdl&use=".$f_u[0]."&did=".$row[0]."&tid=".$key."&fid=".$row[2]."&ti=".str_replace([':'], ['f'], $value)."&token=".substr(md5($f_u[0].$row[0].$key.$row[2].str_replace([':'], ['f'], $value)),1,10)."	' alt='Выписать' title='Выписать' border='0'><img src='template/apimages/delete.png' width='20' height='20'></a>");
                      ...
                      if($f_u[1] == 0 ) {
			                     echo("<td><a  href='index.html&sub=partner&did=".$row[0]."&tid=".$key."&fid=".$row[2]."&ti=".str_replace([':'], ['f'], $value)."&token=".substr(md5($row[0].$key.$row[2].str_replace([':'], ['f'], $value)),1,10)."'><center><span style='color: green;'><i class='fa fa-plus' title='Записать напарника'></i></span></center></a>  </td>");
			                }
                      else {
			                     echo("<td> </td>");
		                  }
        ...
        $user1=mysql_fetch_array(mysql_query("SELECT `name`,`id` FROM `users` WHERE `id`='".$f_u[1]."'"));
	                   ...
	                   echo("</th><td bgcolor='#90EE90'> ".$user0[0]."");
                     ...
                     if($user0[0]!=$user_arr[0] ){
	                   ...
	                   echo("</td><td bgcolor='#90EE90'> ".$user1[0]."");

	                   if($user1[0] != $user_arr[0] ){
						  if($cfg[0]==1 or $user_arr[4]!=1){
                                echo(" <a href='stand.html&sub=mdl&use=".$f_u[1]."&did=".$row[0]."&tid=".$key."&fid=".$row[2]."&ti=".str_replace([':'], ['f'], $value)."&token=".substr(md5($f_u[1].$row[0].$key.$row[2].str_replace([':'], ['f'], $value)),1,10)."' alt='Выписать' title='Выписать' border='0'><img src='template/apimages/delete.png' width='20' height='20'></a> ");
...
//	 echo("
//		<td >
//		<a href='index.html&sub=delete&use=".$user_arr[0]."&did=".$row[0]."&tid=".$key."&fid=".$row[2]."&ti=".str_replace([':'], ['f'], $value)."' alt='Выписаться' title='Выписаться' border='0'><img src='template/apimages/delete.png' width='20' height='20'></a>
//		</td>
//		");
//	  }
