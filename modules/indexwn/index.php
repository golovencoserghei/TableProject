...
if(auth===FALSE)
{
    ...
    $err="Ошибка выбора таблицы!";
    msg($err,"wr");

  }
	$weak=mysql_fetch_array(mysql_query("SELECT * FROM `tb_congr` WHERE `id`='".$tbid."'"));
...
if($e == 0){
	...
	$finv=explode("-",$_SESSION['fdwn']);
	 $finvers_date=$finv[2].'-'.$finv[1].'-'.$finv[0];
	$linv=explode("-",$_SESSION['ldwn']);
...
?>
					<ul class="nav nav-tabs  nav-tabs-line">
                                    <li class="nav-item">
                                        <a href="indexn.html" class="nav-link">Эта неделя</a>
                                    </li>
                                    <li class="nav-item">
                                        <a href="#neww" data-toggle="tab" class="nav-link active">Следующая неделя</a>
                                    </li>
                                    <li class="nav-item">
...
<div class='tab-pane fade show active' id='neww'>
	...
	{
	 $one=mysql_fetch_array(mysql_query("SELECT `su_time`,`mo_time`,`tu_time`,`we_time`,`th_time`,`fr_time`,`sa_time`,`su_time`,`stat_add_us` FROM `tb_congr` WHERE `id`='".$tbid."'"));
	 //$stat_add_us=mysql_fetch_array(mysql_query("SELECT `stat_add_us` FROM `tb_congr` WHERE `id`='$tbid'"));
     $wday='';
	 if($row[2]==1){
		 $wday="Понедельник";
	 }
   ...
   ");
    ...
    foreach($one as $key => $value)
	{
		...
		$us_name=mysql_fetch_array(mysql_query("SELECT `name`,`id` FROM `users` WHERE `id`='".$f_u[1]."'"));
		   echo("
				...
				if($f_u[1]!=0){
						echo("<a href='indexn.html&sub=delete&use=".$f_u[1]."&did=".$row[0]."&tid=".$key."&fid=".$row[2]."&pag=nw&ti=".str_replace([':'], ['f'], $value)."' alt='Выписать' title='Выписать' border='0'><img src='template/apimages/delete.png' width='20' height='20'></a>");
					}
		...
		echo("
		  ...
		  if($f_u[0] == 0) {
			 echo("
			        <td><a  href='indexn.html&sub=partner&did=".$row[0]."&tid=".$key."&fid=".$row[2]."&pag=nw&ti=".str_replace([':'], ['f'], $value)."'><center><span style='color: green;'><i class='fa fa-plus' title='Записать напарника'></i></span></center></a>
			     </td>
			     ");
		  }
		...
		$us_name=mysql_fetch_array(mysql_query("SELECT `name`,`id` FROM `users` WHERE `id`='".$f_u[0]."'"));
			echo("
				...
				if($f_u[0]!=0){
						...
						echo("<a href='indexn.html&sub=delete&use=".$f_u[0]."&did=".$row[0]."&tid=".$key."&fid=".$row[2]."&pag=nw&ti=".str_replace([':'], ['f'], $value)."	' alt='Выписать' title='Выписать' border='0'><img src='template/apimages/delete.png' width='20' height='20'></a>");
					}
			...
			if($f_u[1] == 0 ) {
			 echo("
			        <td><a  href='indexn.html&sub=partner&did=".$row[0]."&tid=".$key."&fid=".$row[2]."&pag=nw&ti=".str_replace([':'], ['f'], $value)."'><center><span style='color: green;'><i class='fa fa-plus' title='Записать напарника'></i></span></center></a>  </td>
			     ");
			}
			else {
			  echo("
	...
	echo("<a  href='indexn.html&sub=raport&did=".$row[0]."&tid=".$key."'><i class='icon icon-notebook2 green-text s-18'></i></a>");
	  }
  ...
  //  if($f_u[0] == $user_arr[0] or $f_u[1] == $user_arr[0]){
	...
	//  echo("
	// 	<td >
	// 	<a href='indexn.html&sub=delete&use=".$user_arr[0]."&did=".$row[0]."&tid=".$key."&fid=".$row[2]."&pag=nw&ti=".str_replace([':'], ['f'], $value)."' alt='Выписаться' title='Выписаться' border='0'><img src='template/apimages/delete.png' width='20' height='20'></a>
	// 	</td>
	// 	");
	//   }
	//
   ...
   echo("  </tbody>
		...
		echo"<br><div class='tab-pane fade show active' id='neww'>
	 <div class='records--list' data-title='Product Listing'>
	<table class='table table-bordered'>
     <thead>
      <tr>
          <th bgcolor='#FA8072' >
				...
				<tbody>
					<tr><th bgcolor='#98FB98' >Таблица на следующею неделю, будет доступной в ";
