...
if( $user_arr['group']==4 or $user_arr['group']==9 ){
	...
	$user = (int) dateprotect($get_array['u']);
	...
	$post_nrp = dateprotect($post_array['nrp']);
...
if (count($post_array) != 0){
	...
	$grp=mysql_fetch_array(mysql_query("SELECT * FROM `tb_group` WHERE `uid`='".$user."'"));
	
	$pgrp=mysql_fetch_array(mysql_query("SELECT * FROM `tb_group` WHERE `gid`='".$grp['gid']."' and `prim`='".$grp['gid']."'"));
	
	$rap=mysql_query("SELECT * FROM `".$rap_bd."` WHERE `uid`='".$user."' LIMIT 1");
	...
	$inf_rap=mysql_fetch_array(mysql_query("SELECT * FROM `".$rap_bd."` WHERE `uid`='".$user."'"));
	
	
?>


<a class="btn btn-violet" href='report.html&sub=rp&mn=<? echo $getm;?>'> << Назад</a> 
<a class="btn btn-violet" href='report.html&sub=cart&u=<? echo $user;?>'> Карочка возвещатея</a>
<div class='d-flex align-items-center justify-content-between mt-40 mb-20'>
				<h4>Отчет <? echo $user_vinf['name'];    ?></h4>
                                    ...
                                    <div class="row">
										...
										<div class="form-group">
                                            ...
                                            <div class="input-group">
												...
												<input type="text" class="form-control"  value="<? echo nmonth($getm);?>" readonly>
										...
										<div class="form-group">
                                            <div class="input-group">
                                                ...
                                                <input type="number" name='p' class="form-control"  value='<? echo $inf_rap['pb'];?>' placeholder="0" aria-label="Default" aria-describedby="inputGroup-sizing-default">
                                            </div>
                                        </div>
										...
										<div class="form-group">
                                            <div class="input-group">
                                                ...
                                                <input type="number" name='pv' class="form-control" placeholder="0"  value='<? echo $inf_rap['vd'];?>' aria-label="Default" aria-describedby="inputGroup-sizing-default">
                                            </div>
                                        </div>
										...
										<div class="form-group">
                                            <div class="input-group">
                                                ...
                                                <input oninput="up(this)" id="stime" type="text" name='t'  value='<? echo $inf_rap['tm'];?>' class="form-control" placeholder="0" aria-label="Default" aria-describedby="inputGroup-sizing-default">
                                            </div>
                                        </div>
										...
										<div class="form-group">
                                            <div class="input-group">
                                                ...
                                                <input type="number" name='pp' class="form-control" placeholder="0"  value='<? echo $inf_rap['pp'];?>' aria-label="Default" aria-describedby="inputGroup-sizing-default">
                                            </div>
                                        </div>
										...
										<div class="form-group">
                                            <div class="input-group">
                                                ...
                                                <input type="number" name='i' class="form-control" placeholder="0"   value='<? echo $inf_rap['iz'];?>' aria-label="Default" aria-describedby="inputGroup-sizing-default">
                                            </div>
                                        </div>
										...
										<div class="form-group">
                                            <div class="input-group">
                                                ...
                                                <input type="text" name='info'   value='<? echo $inf_rap['inf'];?>' class="form-control"  aria-label="Default" aria-describedby="inputGroup-sizing-default">
                                            </div>
                                        </div>
										
										<div class="form-group">
                                           <div class="custom-control custom-checkbox">
                                                <input name="nrp" value="5" type="checkbox" class="custom-control-input" id="customCheck1" <? if ($inf_rap['r_type']==1) { echo "checked";} ?> >
                                                <label class="custom-control-label" for="customCheck1">Нет отчета</label>
                                            </div>
                                        </div>
										...
										<div class="form-group">
                                           <div class="custom-control custom-checkbox">
                                                <input name="pioner" value="1" type="checkbox" class="custom-control-input" id="customCheck2" <? if ($inf_rap['pp_op']==1) { echo "checked";} ?>>
                                                <label class="custom-control-label" for="customCheck2">Подсобный Пионер</label>
                                            </div>
                                        </div>
										<div class="form-group">
                                           <div class="custom-control custom-checkbox">
                                                <input name="pioner" value="2" type="checkbox" class="custom-control-input" id="customCheck3" <? if ($inf_rap['pp_op']==2) { echo "checked";} ?>>
                                                <label class="custom-control-label" for="customCheck3">Общий пионер</label>
                                            </div>
                                        </div>
...
<div class='d-flex align-items-center justify-content-between mt-40 mb-20'>
				<h4>История отчетов за <? echo $year; ?> служебный года</h4>
...
<?	
	...
	if ($rap_sp['iz']==0){
		...
		$iz+=$rap_sp['iz'];
		echo("<th ".$bgcolor.">".$rap_sp['iz']."</th>");
	}
	
	echo("<th ".$bgcolor.">".$rap_sp['inf']."</th>");
